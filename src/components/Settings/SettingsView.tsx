...
export const SettingsView: React.FC = () => {
  ...
  const formatUnit = (unit: string) => {
    ...
    if (u === "bộ") return "set";
    if (u === "kg") return "kg";
    if (u.includes("người") || u.includes("khách")) return "guest/night";
    if (u.includes("chuyến")) return "trip";
    if (u.includes("chiếc")) return "vehicle/day";
    if (u.includes("lượt")) return "entry";
    if (u.includes("trang")) return "page";
    return unit;
  };
  ...
  const getLocalizedServiceDesc = (desc?: string) => {
    ...
    if (desc.includes("minibar") && desc.includes("Nước suối")) return "Bottled mineral water in room minibar";
    if (desc.includes("thiên nhiên") || desc.includes("có ga")) return "Natural sparkling mineral water";
    if (desc.includes("giải khát")) return "Chilled soft drinks and beverages";
    if (desc.includes("ướp lạnh") || desc.includes("minibar")) return "Chilled room minibar item";
    if (desc.includes("trong ngày") || desc.includes("12:00")) return "Standard folded laundry service (same day)";
    if (desc.includes("chất liệu") || desc.includes("giặt hấp")) return "Premium care dry cleaning & ironing";
    if (desc.includes("trang phục cao cấp")) return "Dry cleaning for delicate formal wear";
    if (desc.includes("dưới 2 tiếng")) return "Late checkout fee for stays under 2 hours";
    if (desc.includes("50%")) return "Standard 50% room rate surcharge";
    if (desc.includes("100%")) return "Full 100% current night room rate charge";
    if (desc.includes("5 sao") || desc.includes("nệm")) return "Includes 5-star standard bedding & mattress";
    if (desc.includes("ngoài định mức") || desc.includes("phát sinh")) return "Extra adult guest fee beyond standard occupancy";
    if (desc.includes("trẻ nhỏ")) return "Soft bedding & blanket set for young children";
    if (desc.includes("sân bay") || desc.includes("Tân Sơn Nhất")) return "Airport transfer pick-up & drop-off service";
    if (desc.includes("mũ bảo hiểm") || desc.includes("du lịch")) return "Includes 2 helmets & local travel map";
    if (desc.includes("khử trùng") || desc.includes("vệ sinh")) return "Specialized deep-cleaning & sanitization fee";
    if (desc.includes("lễ tân") || desc.includes("in ấn")) return "Front desk printing & photocopying assistance";
    return desc;
  };
